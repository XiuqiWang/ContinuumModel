...
def calc_Mdrag_params(c, Uair, U):
    alpha_drag = 0.32
    Ngrains = c/mp
...
def MD_eff_params(Ua, U, c, params):
    _Pmax, _Uc, _pshape, _A_NE, _A_e, B, p = params
    # alpha, K = 1.20, 0.040
    # Uaeff = alpha*Ua
    # MD_eff = rho_a * K * c/(rho_p*d) *abs(Uaeff - U) * (Uaeff - U)
    Mdrag = calc_Mdrag_params(c, Ua, U)
    CD_bed = 0.0037
    # B, p = 4.46, 7.70
...
def Preff_of_U_param(U, params):
    Pmax, Uc, pshape, _A_NE, _A_e, B, p = params
    Pmin = 0.0
    Uabs = np.abs(U)
    return Pmin + (Pmax - Pmin)*(1.0 - np.exp(-(Uabs/Uc)**pshape))  

def NE_from_Uinc(Uinc, params):
    Pmax, Uc, pshape, A_NE, _A_e, B, p = params
...
def e_COR_from_Uim(Uim, params):
    Pmax, Uc, pshape, _A_NE, A_e, B, p = params
    # e = 3.05 * (abs(Uim)/const + 1e-12)**(-0.47)
    e = A_e * (abs(Uim)/const + 1e-12)**(-0.47)
    return e      
...
def rhs(t, y, eps, u_star, params):
    ...
    eCOR = e_COR_from_Uim(Uim, params); Ure = Uim*eCOR
    ...
    M_drag = calc_Mdrag_params(c, Ua, U) # changed
...
def simulate_and_sample_case(u_star_case, params, T, y0, dt):
    t, y_samp = euler_forward(rhs, y0, (0.0, T), dt, u_star_case, params)
    c_mod, U_mod, Ua_mod = y_samp[0], y_samp[1]/np.maximum(y_samp[0], 1e-16), y_samp[2]
...
p0 = np.array([0.75, 2.0, 0.4, 0.03, 4.0, 4.5, 7.7])    # [Pmax, Uc, p, A_NE, A_e, B, p]
lb = np.array([0.3, 0.1, 0.2, 0.01, 3.0, 0.1, 0.1])
ub = np.array([1.0, 10.0, 3.0, 0.1, 5.0, 10.0, 20.0])
