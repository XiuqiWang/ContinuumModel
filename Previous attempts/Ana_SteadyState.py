...
def MD_eff(Ua, U, c):
    Mdrag = calc_Mdrag_overc(Ua, U) * c # times c here to make it c-dependent
    CD_bed = 0.0037
    B, p = 4.46, 7.70
...
def Cal_EDM(U):
    ...
    M_bed_overc = M_dep_overc + M_im_overc - M_re_overc - M_eje_overc
    
    return E_overc, D_overc , M_bed_overc

#----------- DPM data ----------
cases = range(2, 7)  # S002..S006
C_list, U_list, Ua_list, MD_list, MDeff_list, E_list, D_list = [], [], [], [], [], [], []
...
for i in cases:
    ...
    Ua_list.append(ua[:, 1])
    # Mdrag
    MD = np.loadtxt(f"TotalDragForce/FD_S{i:03d}dry.txt")
    MD_list.append(MD)
    # Mdrag_eff
    phi = cg[:, 1]/(rho_p*h)
    dUa_dt = np.gradient(ua[:, 1], dt)
    tau_top = rho_a * u_star[i-2]**2
    MDeff = tau_top - rho_a * h * dUa_dt * (1-phi)
    MDeff_list.append(MDeff)
    # E and D
...
for i in range(5):
    ...
    # Steady Ua
    dU_com = 0.32*Ua_com - Usteady[i]
    _,_,M_bed_steady_overc = Cal_EDM(Usteady[i])
    M_drag_overc = calc_Mdrag_overc(Ua_com, Usteady[i])   
    Uasteady[i] = np.interp(M_bed_steady_overc, M_drag_overc, Ua_com)
...
dU_emp = 0.32*Ua_emp - U_emp
Mdrag_overc_emp = MD_list[4]/C_emp
dcUdt_emp = np.gradient(U_emp*C_emp, dt)
Mbed_overc_emp = (MD_list[4] - dcUdt_emp)/C_emp
MDeff_emp = MDeff_list[4]
...
plt.plot(dU_com, M_bed_steady_overc*np.ones_like(dU_com), color='C1', label='M_bed/c')
plt.plot(dU_emp, Mdrag_overc_emp, '.', color='C0', label='M_drag/c DPM')
plt.plot(dU_emp, Mbed_overc_emp, '.', color='C1', label='M_bed/c DPM')
plt.xlabel('0.32Ua - U [m/s]')
plt.ylabel('M/c')
...
plt.plot(C_emp, MDeff_emp, '.', color='C0', label=r'$M_{D,eff}$ DPM')
