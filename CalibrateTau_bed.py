...
for i in range(5): #Omega
    ...
    for j in range(5): #Shields
        plt.subplot(3, 2, j+1)
        index_byS = i*5+j 
        plt.plot(t, RHS_all_S[index_byS], '.', label=r'$\hat{M}_\mathrm{creep}=\hat{M}_\mathrm{top,air} -\rho_\mathrm{air}h(1-\frac{\hat{c}}{m_\mathrm{p}h})\frac{d\hat{U}_\mathrm{air}}{dt} - \hat{M}_{sal} - \hat{M}_\mathrm{bed,air}$')
        Mcreep = CalMcreep(MD_all_S[index_byS], gamma)
        plt.plot(t, Mcreep, '.', label=fr'$\breve{{M}}_{{\mathrm{{creep}}}} = {gamma:.2f}\,\hat{{M}}_{{\mathrm{{sal}}}}$')
        plt.title(fr"$\tilde{{\Theta}}$=0.0{j+2}", fontsize=14)
        plt.xlabel(r"$t$ [s]", fontsize=14)
        plt.ylabel(r"$M_\mathrm{creep}$ [N/m$^2$]", fontsize=14)
        plt.ylim(0,2.5)
        plt.xlim(0,5)
        plt.grid(True)
        if j == 0:
            plt.legend(fontsize=10, loc='upper right')
    plt.suptitle(fr'$\Omega$={Omega[i]}%', fontsize=14)
    plt.tight_layout()
    plt.show()
