...
def rhs_cmUa(t, y, u_star, Omega, eps=1e-16):
    ...
    eCOR = e_COR_from_Uim_test(Uim, Omega); Ure = Uim*eCOR
...
E_on_U = np.interp(t_dpm, t_Dis, E_dpm)
D_on_U = np.interp(t_dpm, t_Dis, D_dpm)
E_dpm_overc = E_on_U/C_dpm
D_dpm_overc = D_on_U/C_dpm
...
E_overc = E_all/c[:-1]
D_overc = D_all/c[:-1]
Mdrag_overc = Mdrag_all/c[:-1]
Mbed_overc = Mbed_all/c[:-1]

plt.close('all')
plt.figure(figsize=(15,4))
plt.subplot(1,3,1)
plt.plot(U[:-1], E_overc, color='C0', label='E/c')
plt.plot(U[:-1], D_overc, color='C1', label='D/c')
plt.plot(U_dpm, E_dpm_overc, 'C0.', label='E/c DPM')
plt.plot(U_dpm, D_dpm_overc, 'C1.', label='D/c DPM')
plt.xlabel('U [m/s]')
plt.ylabel('E/c & D/c')
plt.legend()
plt.subplot(1,3,2)
plt.plot(dU[:-1], Mdrag_overc, color='C0', label='M_drag/c')
plt.plot(dU[:-1], Mbed_overc, color='C1', label='M_bed/c')
...
plt.legend()
plt.subplot(1,3,3)
plt.plot(c[:-1], Mdrageff_all, color='C0', label=r'$M_{drag,eff}$')
plt.plot(c, tau_top_emp*np.ones_like(c), color='C1', label='tau_top')
